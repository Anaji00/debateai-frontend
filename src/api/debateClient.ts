...
export async function uploadDoc(session_id: number, file: File, owner = "shared", title?: string) {
...
export async function listDocs(session_id: number) {
...
export async function deleteDocsForOwner(session_id: number, owner: string) {
  const res = await fetch(`${API_BASE}/debate/docs/owner/${session_id}/${encodeURIComponent(owner)}`, withCreds({ method: "DELETE" }));
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
